...
function Autocomplete() {
    ...
    const searchCountries = React.useCallback(async (countries) => {
        ...
        try {
            setIsLoading(true);
            const res = await fetch(`https://freetestapi.com/api/v1/countries?search=${countries}`);
  ...
  return (
    <>
        <h1 className='mb-5'>Autocomplete</h1>
        <div>
            <div className='w-52 h-[38px] border border-gray-900 p-2 rounded-lg flex'>
                <input type="text"
                className='w-40 h-5'
                value={searchTerm}
